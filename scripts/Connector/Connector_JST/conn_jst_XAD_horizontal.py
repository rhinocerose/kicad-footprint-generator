...
#Note : currently only 8-20 and 26pin variants are listed in the datasheet.
...
mh_size = [2.8, 2]	#oval mounting hole, NPTH
mh_drill = [2.8, 2]
mh_y = -7.6

pin_range = list(range(4,11))+[13] #number of pins in each row

#FP name strings
part_base = "S{n:02}B-XADSS-N" #JST part number format string
...
def generate_one_footprint(pins, configuration):
    ...
    x2 = x1 + B
    y2 = -10.3
    y1 = y2 +16.2	#XAD side-entry (horizontal) : pin 1 is closest to mounting holes
    body_edge={'left':x1, 'right':x2, 'top':y1, 'bottom':y2}

    #draw fab outline measured from 3D model on JST's site
    # "wings" by pins are 1.25mm wide
    wing = 1.25
    wingdepth = 2.9
    side = [
    {'x': x1,'y': y1},
    {'x': x1+wing,'y': y1},
    {'x': x1+wing,'y': y1 - wingdepth},
    {'x': x2-wing,'y': y1 - wingdepth},
    {'x': x2-wing,'y': y1},
    {'x': x2,'y': y1},
    {'x': x2,'y': y2},
    {'x': x1,'y': y2},
    ...
    if y2 < -size[1]/2.0:
        cy1 = roundToBase(y2-configuration['courtyard_offset']['connector'], configuration['courtyard_grid'])
    else:
    ...
    cy2 = roundToBase(y1+configuration['courtyard_offset']['connector'], configuration['courtyard_grid'])
    ...
    side = [
    {'x': x1+wing+2*off,'y': y1 - wingdepth},
    {'x': x1+wing+2*off,'y': y1},
    {'x': x1,'y': y1},
    {'x': x1,'y': y2},
    {'x': xmid - recess/2,'y': y2},
    {'x': xmid - recess/2,'y': y2+1},
    {'x': xmid,'y': y2+1},
    ]

    kicad_mod.append(PolygoneLine(polygone=side, width=configuration['silk_line_width'], layer='F.SilkS'))
    kicad_mod.append(PolygoneLine(polygone=side, x_mirror=xmid, width=configuration['silk_line_width'], layer='F.SilkS'))

    #add mounting holes
    m1 = Pad(at=[0,mh_y],layers=Pad.LAYERS_NPTH,shape=Pad.SHAPE_OVAL,type=Pad.TYPE_NPTH,size=mh_size, drill=mh_drill)
    m2 = Pad(at=[A,mh_y],layers=Pad.LAYERS_NPTH,shape=Pad.SHAPE_OVAL,type=Pad.TYPE_NPTH,size=mh_size, drill=mh_drill)
