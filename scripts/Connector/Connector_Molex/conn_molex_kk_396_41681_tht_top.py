...
orientation = 'V'
number_of_rows = 1
datasheet = 'https://www.molex.com/pdm_docs/sd/026487095_sd.pdf'
...
eng_mpn = 'A-41681-B{n}A197'
new_mpn_example = '26-48-1{n:02d}3'
...
def generate_one_footprint(pincount, configuration):
    ...
    body_edge={
        'left':start_pos_x - 1.955,
        'right':end_pos_x + 1.955,
        'top':-3.81
        }
    body_edge['bottom'] = body_edge['top'] + 7.6 # not given in datasheet but measured
    ...
    # create fab outline and silkscreen
    for nudge, layer, width in zip([0, silk_offset], ['F.Fab', 'F.SilkS'], [fab_w, silk_w]):
        kicad_mod.append(PolygoneLine(polygone=[
            ...
            [body_edge['left'] - nudge, body_edge['top'] - nudge]], layer=layer,
            width=width))
    ...
    poly_pin1_marker = [
        {'x': body_edge['left'] - 0.5, 'y': 0},
        {'x': body_edge['left'] - 1.5, 'y': sl/2},
        {'x': body_edge['left'] - 1.5, 'y': sl/-2},
        {'x': body_edge['left'] - 0.5, 'y': 0}
    ]
    kicad_mod.append(PolygoneLine(polygone=poly_pin1_marker, layer='F.SilkS', width=silk_w))
    ...
    # create silkscreen friction-fit 'fingers'
    for i in range(0, pincount):
        middle_x = start_pos_x + i * pitch
        start_x = middle_x - 1.9/2 # not given in datasheet but measured
        end_x = middle_x + 1.9/2
        y1 = body_edge['bottom'] + silk_offset
        y2 = 2.59 # given in datasheet
