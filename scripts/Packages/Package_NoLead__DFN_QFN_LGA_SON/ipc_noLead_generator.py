...
class NoLead():
        ...
        with open(ipc_doc_file, 'r') as ipc_stream:
            try:
                ...
                if 'ipc_generic_rules' in self.ipc_defintions:
                    self.configuration['min_ep_to_pad_clearance'] = \
                        self.ipc_defintions['ipc_generic_rules'].get(
    ...
    def calcPadDetails(self, device_dimensions, EP_size, ipc_data, ipc_round_base):
        ...
        pad = {
            'left': {
                'center': [-(Zmax_x+Gmin_x)/4, 0],
                'size': [(Zmax_x-Gmin_x)/2, Xmax]
            },
            'right': {
                'center': [(Zmax_x+Gmin_x)/4, 0],
                'size': [(Zmax_x-Gmin_x)/2, Xmax]
            ...
            'top': {
                'center': [0, -(Zmax_y+Gmin_y)/4],
                'size': [Xmax, (Zmax_y-Gmin_y)/2]
            },
            'bottom': {
                'center': [0, (Zmax_y+Gmin_y)/4],
                'size': [Xmax, (Zmax_y-Gmin_y)/2]
    ...
    def deviceDimensions(device_size_data, fp_id):
        ...
        dimensions = {
            'body_size_x': TolerancedSize.fromYaml(
                device_size_data, base_name='body_size_x', unit=unit),
            'body_size_y': TolerancedSize.fromYaml(
                device_size_data, base_name='body_size_y', unit=unit),
            'lead_width': TolerancedSize.fromYaml(
                device_size_data, base_name='lead_width', unit=unit),
            'pitch': TolerancedSize.fromYaml(
                device_size_data, base_name='pitch', unit=unit).nominal
        }
        ...
        if 'EP_size_x_min' in device_size_data and 'EP_size_x_max' in device_size_data or 'EP_size_x' in device_size_data:
            dimensions['EP_size_x'] = TolerancedSize.fromYaml(
                    device_size_data, base_name='EP_size_x', unit=unit)
            dimensions['EP_size_y'] = TolerancedSize.fromYaml(
                    device_size_data, base_name='EP_size_y', unit=unit)
        ...
        if 'lead_to_edge' in device_size_data:
            dimensions['lead_to_edge'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_to_edge',
                unit=unit)

        if 'lead_center_pos_x' in device_size_data:
            dimensions['lead_center_pos_x'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_center_pos_x',
                unit=unit)
        if 'lead_center_to_center_x' in device_size_data:
            dimensions['lead_center_pos_x'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_center_to_center_x',
                unit=unit)/2

        if 'lead_center_pos_y' in device_size_data:
            dimensions['lead_center_pos_y'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_center_pos_y',
                unit=unit)
        if 'lead_center_to_center_y' in device_size_data:
            dimensions['lead_center_pos_y'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_center_to_center_y',
                unit=unit)/2

        dimensions['lead_len_H'] = None
        dimensions['lead_len_V'] = None
        if 'lead_len_H' in device_size_data and 'lead_len_V' in device_size_data:
            dimensions['lead_len_H'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_len_H', unit=unit)
            dimensions['lead_len_V'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_len_V', unit=unit)
        elif 'lead_len' in device_size_data or (
                'lead_len_min' in device_size_data and 'lead_len_max' in device_size_data):
            dimensions['lead_len_H'] = TolerancedSize.fromYaml(
                device_size_data, base_name='lead_len', unit=unit)
        ...
        if 'body_to_inside_lead_edge' in device_size_data:
            dimensions['body_to_inside_lead_edge'] = TolerancedSize.fromYaml(
                device_size_data, base_name='body_to_inside_lead_edge',
                unit=unit)
        elif dimensions['lead_len_H'] is None:
            raise KeyError(
                '{}: Either lead lenght or inside lead to edge dimension must be given.'.format(fp_id))
    ...
    def __createFootprintVariant(self, device_params, device_dimensions, with_thermal_vias):
        ...
        kicad_mod.setTags(self.configuration['keyword_fp_string'].format(
            man=device_params.get('manufacturer',''),
            package=device_params['device_type'],
            category=category
        ).lstrip())
        ...
        pad_radius = add_dual_or_quad_pad_border(
            kicad_mod, self.configuration, pad_details, device_params)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--series_config', type=str, nargs='?',
        help='the config file defining series parameters.',
        default='../package_config_KLCv3.yaml')
    parser.add_argument(
        '--density', type=str, nargs='?',
        help='IPC density level (L,N,M)',
        default='N')
    parser.add_argument(
        '--ipc_doc', type=str, nargs='?',
        help='IPC definition document',
        default='../ipc_definitions.yaml')
    parser.add_argument(
        '--force_rectangle_pads', action='store_true',
        help='Force the generation of rectangle pads instead of rounded rectangle')
    parser.add_argument(
        '--kicad4_compatible', action='store_true',
        help='Create footprints kicad 4 compatible')
    parser.add_argument(
